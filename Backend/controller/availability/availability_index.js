...
router.get('/', (req, res) => {
	const sql = 'SELECT * FROM availability ORDER BY day asc, hour asc';
...
// Simple request to see a users availability
router.get('/:id', (req, res) => {
	const sql = 'SELECT * FROM availability WHERE user_id = ? ORDER BY day asc, hour asc';
...
router.post('/', (req, res) => {
	...
	db.query(sql_get, [body.user_id], (err, row) => {
		if (err) throw err;
		...
		db.query(sql_del, [body.user_id], (err, result) => {
			...
			for(var day in body.hours_available) {
				for(var hour in body.hours_available[day].hour) {
					db.query(sql_ins, [body.user_id, body.hours_available[day].day,
...
// From all players find the 10 players that are the most compatible with the user
router.get('/top10/:id', (req, res) => {
	...
	sql_get = 'SELECT user2 as matched_player, day, hour, skill_diff ' 
	+ 'FROM ('
	...
	+ 'WHERE us1.user_id = ? ORDER BY us2.user_id asc, day asc, hour asc '
	+ ') as Matches '
	...
	db.query(sql_get, [req.params.id], (err, result) => {
		...
		for(var entry in result) {
			...
			if (result[entry].matched_player != last_user) {
